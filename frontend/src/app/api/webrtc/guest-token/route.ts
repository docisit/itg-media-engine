...
export async function POST(request: NextRequest) {
  console.log(">>> GUEST_TOKEN_ROUTE_STARTED");
  ...
  try {
    ...
    console.log(`>>> PROXYING_TO_BACKEND: ${backendUrl}/api/webrtc/guest-token/`);
    console.log(`>>> REQUEST_BODY:`, JSON.stringify(body));

    const response = await fetch(`${backendUrl}/api/webrtc/guest-token/`, {
      method: 'POST',
      headers: {
