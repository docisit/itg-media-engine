...
export async function POST(request: NextRequest) {
  console.log(">>> STREAM_KEY_ROUTE_STARTED");
  ...
  try {
    ...
    console.log(`>>> PROXYING_TO_BACKEND: ${backendUrl}/api/webrtc/stream-key/`);
    console.log(`>>> REQUEST_BODY:`, JSON.stringify(body));

    const response = await fetch(`${backendUrl}/api/webrtc/stream-key/`, {
      method: 'POST',
      headers: {
